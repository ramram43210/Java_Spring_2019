...
public class EmployeeResource
{
	...
	public Employees getAllEmployees()
	{
		...
		{
			// Adding get/self link employee 'singular' resource
			Link getLink = ControllerLinkBuilder.linkTo(EmployeeResource.class).slash("/employees")
					.slash(employee.getEmployeeId()).withSelfRel().withTitle("Get Employee Detail")
					.withType("GET");

			// Adding update link employee 'singular' resource
			Link updateLink = ControllerLinkBuilder.linkTo(EmployeeResource.class)
					.slash("/employees").slash(employee.getEmployeeId()).withRel("update")
					.withTitle("Update Employee Detail").withType("Put");

			// Add link to singular resource
			employee.add(getLink);
			employee.add(updateLink);
		}
